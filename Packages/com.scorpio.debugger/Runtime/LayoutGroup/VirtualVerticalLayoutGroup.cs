...
namespace Scorpio.Debugger {
    ...
    public class VirtualVerticalLayoutGroup : LayoutGroup, IPointerClickHandler {
        ...
        private readonly List<int> _visibleItemList = new List<int>();

        private bool _isDirty = false;
        private List<Row> _rowCache = new List<Row>();
        ...
        private float ItemHeight
        {
            get
            {
                if (_itemHeight <= 0)
                {
                    ...
                    if (Mathf.Approximately(0, _itemHeight))
                    {
                        Debug.LogWarning(
        ...
        private Row GetRow(int forIndex)
        {
            ...
            else if (row == null)
            {
                ...
                row = _rowCache[_rowCache.Count - 1];
